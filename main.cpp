...
std::string describe(CLightRef tl) {
    ...
    // Question: Why does the 'advance' function take a reference, while 'describe' takes a const reference? -> 'advance' needs to modify the object's state, so it takes a mutable reference (LightRef). 'describe' only needs to read the state without changing it, so it takes a const reference (CLightRef) for safety and efficiency.
    std::string s = "Light is " + to_string(tl.state) +
                    ", duration " + std::to_string(tl.duration_sec) +
                    "s, pedestrian mode is " + (tl.pedestrian_mode ? "ON" : "OFF");
    return s;
}
